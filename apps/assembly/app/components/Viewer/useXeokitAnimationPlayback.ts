/**
 * Animation playback engine for assembly steps (xeokit version).
 *
...
export interface UseXeokitAnimationPlaybackOptions {
...
export interface XeokitAnimationPlaybackState {
...
export function useXeokitAnimationPlayback({
  viewer,
  isModelLoaded = false,
  steps,
  selectedStepIndex,
...
}: UseXeokitAnimationPlaybackOptions): XeokitAnimationPlaybackState {
    ...
    (stepIdx: number, t: number) => {
      ...
      for (const partId of step.partIds) {
        ...
        if (partKfs && partKfs.length >= 2) {
          ...
          // Single keyframe → offset scaled by (1-t) to animate to rest
          entity.offset = [
