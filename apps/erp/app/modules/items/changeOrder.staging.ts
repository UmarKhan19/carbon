...
// Fetch the minimal item labels for a set of rows and index them by item id.
// Reads use a flat select + JS stitch (no composite-FK PostgREST embeds — the
// erp TS2589 instantiation budget; see lessons). `itemIds` are extracted from
// the caller's rows; empty input short-circuits to an empty map.
