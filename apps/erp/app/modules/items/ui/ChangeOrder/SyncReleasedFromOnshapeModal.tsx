...
// Release-picker modal (Task 18). Two-step flow inside one modal:
//   1. pick a connected OnShape document (reuses the documents browse fetcher
//      pattern from OnshapeSync.tsx)
//   2. pick a released revision from that document (onShapeReleasedRevisions
//      loader) and import it.
// On import success the action returns the auto-created Draft change order's
// row uuid; we navigate to it (the Draft-CO landing). On refusal (e.g. a
// released object with no Part Number) we surface the server message as a toast.
...
const SyncReleasedFromOnshapeModal = ({
  ...
  // Step 1 — documents (mirrors OnshapeSync.tsx documents fetcher shape).
  ...
  // Step 2 — released revisions for the chosen document.
  ...
  // Step 3 — import the selected release.
  ...
  useEffect(() => {
    ...
    if (importFetcher.data.success === false) {
      ...
      // Surface NON-FATAL warnings (e.g. a drawing/geometry pull that was skipped
      // or is still processing) so the operator knows the SSOT files did not
      // attach — the ECO + BOM still landed (Task 26 · surface warnings).
