...
import { changeOrderBomChangeValidator } from "../../changeOrder.models";
...
export default function BomChanges({
  ...
  rows: BomChangeRow[];
  // Per Delete rowId → the assembly picker options (assemblies that actually
  // consume the part). The Delete picker is restricted to these; no free-text.
  suggestedAssembliesByRow?: Record<string, { value: string; label: string }[]>;
  isDisabled: boolean;
}) {
      ...
      <CardContent>
        <VStack spacing={4}>
          <DeleteSection
            ...
            rows={deleteRows}
            suggestedAssembliesByRow={suggestedAssembliesByRow}
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row) => (
          <BomChangeDeleteRow
            ...
            changeOrderId={changeOrderId}
            row={row}
            assemblyOptions={suggestedAssembliesByRow?.[row.id] ?? []}
            isDisabled={isDisabled}
          />
        ))
      )}
