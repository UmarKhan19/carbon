...
import type { PriceTraceStep } from "~/modules/pricing";
...
/**
 * Click-triggered popover that shows the full price resolution trace as a
 * ledger-style table. Designed to fit in a FormLabel right-side slot
 * (where `isOptional` lives) so it doesn't shift the underlying input.
 *
 * Returns null when there is no price list — the caller can render this
 * unconditionally and trust it to be a no-op for non-priced items.
 */
export function PriceTracePopover({
  ...
  // No trace data → just show the source attribution as static text (or link).
  if (steps.length === 0) {
    return (
      <PriceListSourceLink
...
function PriceListSourceLink({
  ...
  return (
    <Link
      to={path.to.priceList(priceListId)}
      target="_blank"
      rel="noreferrer"
...
function ChangeCell({
  ...
  // Negative numbers already render with a leading "-", so only add "+" for surcharges
