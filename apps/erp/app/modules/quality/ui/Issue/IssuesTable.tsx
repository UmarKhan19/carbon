...
type IssuesTableProps = {
  ...
  types: ListItem[];
  count: number;
  containmentStatuses: Record<string, string>;
};

const IssuesTable = memo(
  ({ data, types, count, containmentStatuses }: IssuesTableProps) => {
    ...
    const columns = useMemo<ColumnDef<Issue>[]>(() => {
      const defaultColumns: ColumnDef<Issue>[] = [
        ...
        },
        {
          ...
          cell: ({ row }) => {
            const status =
              containmentStatuses[row.original.id!] ?? "Uncontained";
            return (
    ...
    }, [containmentStatuses, customColumns, items, locations, people, types]);
