  ...
  >(() => {
    return (
      quoteLines?.reduce<Record<string, SelectedLine>>(
        (
          acc: Record<string, SelectedLine>,
          line: QuoteData["quoteLines"][number]
        ) => {
          const salesOrderLine = salesOrderLines?.find(
            (
              salesOrderLine: NonNullable<QuoteData["salesOrderLines"]>[number]
          ...
          const price = salesOrderLine
            ? quoteLinePrices?.find(
                (price: NonNullable<QuoteData["quoteLinePrices"]>[number]) =>
            ...
            line.quantity?.reduce(
              (acc: Record<number, number>, quantity: number) => {
                const charges = Object.values(
                  (line.additionalCharges ?? {}) as Record<
                    string,
                    { amounts?: Record<number, number> }
                  >
                ).reduce((chargeAcc: number, charge) => {
          ...
          const convertedAdditionalChargesByQuantity =
            ...
            >(
              (acc, [quantity, amount]) => {
                acc[Number(quantity)] =
                  Number(amount) * (quote.exchangeRate ?? 1);
                return acc;
              },
              {} as Record<number, number>
            ) ?? {};
