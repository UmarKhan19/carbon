...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    // Task 8a — persist the OnShape company/account id (cid). The Revisions API needs the
    // OnShape cid, which is NOT otherwise stored. Resolve it from getCompanies()[0] at
    // connect time. Additive: never let a lookup failure break the connect flow.
