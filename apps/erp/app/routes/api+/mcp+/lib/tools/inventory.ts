...
export const registerInventoryTools: RegisterTools = (server, ctx) => {
  ...
  server.registerTool(
    "inventory_getNearExpiryTrackedEntities",
    {
      description: "get near expiry tracked entities",
      inputSchema: {
    ...
    withErrorHandling(async (params) => {
      const result = await getNearExpiryTrackedEntities(ctx.client, ctx.companyId, params.thresholdDays);
      return toMcpResult(result);
    }, "Failed: inventory_getNearExpiryTrackedEntities"),
  );
