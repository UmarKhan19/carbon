...
- **git-diff / clobber** (cross-branch): flag any full-redefinition DB object (view, function, `attach_event_trigger`, RLS policy) **redefined on BOTH the PR branch and `main` since their merge-base**. Independent redefinitions live in separate migration files, so git merges with no conflict but the later apply silently clobbers the earlier → regression. Detection is content-agnostic (just *which* objects each side redefines). Pure core `findClobbers(branchFiles, mainFiles)` is testable without git; a thin git CLI supplies the two file sets at PR time. Generalizes the [[feedback_view_redefinition]] / [[feedback_migration_function_redefinition]] / event-trigger-detach rules into an executable gate.
