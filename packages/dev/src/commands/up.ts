  ...
  await tasks([
    {
      task: async () => {
        ...
        ctx = { branchPrefix, root, slug, ...slot };

        writeEnv(root, renderEnv({ branchPrefix, slug, ...slot }));
        syncAppPortlessConfigs({
          branchPrefix,
          linked,
          worktreeRoot: root
        });
        loadDotenv({ override: false, path: join(root, ".env.local") });
        loadDotenv({ override: false, path: join(root, ".env") });
...
async function pullImages(ctx: Ctx) {
  ...
  const bar = progress({ max, style: "heavy" });
...
async function waitForServices(ctx: Ctx) {
  const bar = progress({ max: 5, style: "heavy" });
...
async function runAppsThenTeardown(root: string, selectedApps: AppId[]) {
  await spawnApps({ apps: selectedApps, root });
