...
export async function resolveSlot(
  ...
  registry[slug] = { jwt, ports, redisDb, worktreeRoot };
  writeRegistry(registry);
  return { jwt, ports, redisDb };
}
...
function collectClaims(
  ...
  return { claimedDbs, claimedPorts };
}
...
function generateJwtCreds(): JwtCreds {
  ...
  const anonKey = signJwt(
    { exp, iat, iss: "supabase-demo", role: "anon" },
    secret
  );
  const serviceKey = signJwt(
    { exp, iat, iss: "supabase-demo", role: "service_role" },
    secret
  );
  return { anonKey, secret, serviceKey };
}
