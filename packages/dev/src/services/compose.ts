...
export async function pullStack(
  ...
  const proc = execa(
    ...
    { all: true, cwd: root, reject: false }
  );
...
export async function stopStack(
  ...
  await execa("docker", args, { cwd: root, reject: false, stdio: "ignore" });
}
...
async function execStrict(cmd: string, args: string[], cwd: string) {
  const r = await execa(cmd, args, { cwd, preferLocal: true, reject: false });
  if (r.exitCode !== 0) {
