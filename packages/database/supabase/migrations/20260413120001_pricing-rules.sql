...
-- Item Sale Price Breaks
-- Quantity-based price breaks that live on items (not on pricing rules).

CREATE TABLE "itemSalePriceBreak" (
  "id" TEXT NOT NULL DEFAULT id('ispb'),
  "itemId" TEXT NOT NULL,
  "customerTypeId" TEXT,
  "minQuantity" NUMERIC(20, 2) NOT NULL DEFAULT 1,
  "unitPrice" NUMERIC(15, 5),
  "discountPercent" NUMERIC(10, 5),
  "companyId" TEXT NOT NULL,
  "createdBy" TEXT NOT NULL,
  "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  "updatedBy" TEXT,
  ...
  CONSTRAINT "itemSalePriceBreak_pkey" PRIMARY KEY ("id"),
  CONSTRAINT "itemSalePriceBreak_itemId_fkey"
    FOREIGN KEY ("itemId") REFERENCES "item"("id")
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT "itemSalePriceBreak_customerTypeId_fkey"
    FOREIGN KEY ("customerTypeId") REFERENCES "customerType"("id")
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT "itemSalePriceBreak_companyId_fkey"
    FOREIGN KEY ("companyId") REFERENCES "company"("id")
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT "itemSalePriceBreak_createdBy_fkey"
    FOREIGN KEY ("createdBy") REFERENCES "user"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT "itemSalePriceBreak_updatedBy_fkey"
    FOREIGN KEY ("updatedBy") REFERENCES "user"("id")
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT "itemSalePriceBreak_price_check" CHECK (
    (("unitPrice" IS NOT NULL)::integer + ("discountPercent" IS NOT NULL)::integer) = 1
  )
);

CREATE INDEX "itemSalePriceBreak_itemId_idx" ON "itemSalePriceBreak" ("itemId");
CREATE INDEX "itemSalePriceBreak_customerTypeId_idx" ON "itemSalePriceBreak" ("customerTypeId");
CREATE INDEX "itemSalePriceBreak_companyId_idx" ON "itemSalePriceBreak" ("companyId");

ALTER TABLE "itemSalePriceBreak" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "SELECT" ON "public"."itemSalePriceBreak"
FOR SELECT USING (
  "companyId" = ANY (
    (SELECT get_companies_with_employee_role())::text[]
  )
...
CREATE POLICY "INSERT" ON "public"."itemSalePriceBreak"
FOR INSERT WITH CHECK (
  "companyId" = ANY (
...
CREATE POLICY "UPDATE" ON "public"."itemSalePriceBreak"
FOR UPDATE USING (
  "companyId" = ANY (
...
CREATE POLICY "DELETE" ON "public"."itemSalePriceBreak"
FOR DELETE USING (
  "companyId" = ANY (
