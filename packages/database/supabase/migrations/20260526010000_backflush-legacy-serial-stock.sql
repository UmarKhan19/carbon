...
BEGIN
  ...
  -- Backflush non-tracked materials, plus tracked materials that still have
  -- legacy on-hand stock without tracked entities.
  FOR v_material IN
    SELECT jm.id, jm."itemId", jm."quantityToIssue", jm."quantityIssued",
           jm."estimatedQuantity", jm."storageUnitId", jm."defaultStorageUnit",
           jm."requiresBatchTracking", jm."requiresSerialTracking"
    FROM "jobMaterial" jm
    WHERE jm."jobId" = p_job_id
      ...
      AND (
        (
          jm."requiresBatchTracking" = false
        ...
        OR EXISTS (
          ...
          WHERE il."itemId" = jm."itemId"
            AND il."locationId" = v_job_location_id
  ...
  LOOP
    ...
    IF v_material_storage_unit_id IS NULL THEN
      ...
      WHERE "itemId" = v_material."itemId"
        ...
        AND (
          (
            v_material."requiresBatchTracking" = false
            AND v_material."requiresSerialTracking" = false
          )
          OR "trackedEntityId" IS NULL
        )
