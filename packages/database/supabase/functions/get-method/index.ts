          ...
          async function traverseMethod(
            ...
            for await (const op of relatedOperations?.data ?? []) {
              ...
              jobOperationsInserts.push({
                ...
                operationOrder,
                operationType,
                operationKind: op.operationKind,                operationSupplierProcessId: op.operationSupplierProcessId,
          ...
          async function traverseMethod(
            ...
            const jobOperationsInserts =
              relatedOperations?.data?.map((op) => ({
                ...
                operationKind: op.operationKind,                operationUnitCost: op.operationUnitCost ?? 0,
        ...
        await db.transaction().execute(async (trx) => {
          ...
          jobOperations.data?.forEach((op) => {
            operationInserts.push({
              ...
              operationKind: op.operationKind ?? "Operation",              operationMinimumCost: op.operationMinimumCost ?? 0,
        ...
        await db.transaction().execute(async (trx) => {
          ...
          jobOperations.data?.forEach((op) => {
            operationInserts.push({
              ...
              operationLeadTime: op.operationLeadTime ?? 0,
              operationUnitCost: op.operationUnitCost ?? 0,
              operationKind: op.operationKind ?? "Operation",              operationSupplierProcessId: op.operationSupplierProcessId,
              tags: op.tags ?? [],
              workInstruction: parts.workInstructions ? op.workInstruction : {},
              companyId,
              createdBy: userId,
