...
- `supabase/functions/lib/logging.ts` — Deno-native logger (`getFunctionLogger`, `withRequestLogging`) mirroring `@carbon/logging`; use it instead of `console.*` in edge functions (`@logtape/*` via `deno.json` jsr imports)
