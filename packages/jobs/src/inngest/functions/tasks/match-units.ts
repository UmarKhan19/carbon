...
/**
 * Assigns each distinct CAD part name to the BOM line it belongs to, using an
...
const assignmentSchema = z.object({
  assignments: z.array(
    z.object({
      name: z.string().describe("The exact CAD part name"),
      itemId: z
        .string()
        .nullable()
        .describe("The BOM item id this part belongs to, or null if none fits"),
      confidence: z.number().min(0).max(1)
    })
  )
});

type PartName = { name: string; count: number };
type BomLine = { itemId: string; name: string | null };
export type PartMatch = { name: string; itemId: string };

export async function assignPartsToBom(
  parts: PartName[],
  bom: BomLine[]
): Promise<PartMatch[]> {
  if (parts.length === 0 || bom.length === 0) return [];
...
Return one assignment per CAD part name. Only use BOM item ids from the list above.`
    ...
    console.error("assignPartsToBom failed; leaving parts unmatched:", error);
    return [];
  }
}
