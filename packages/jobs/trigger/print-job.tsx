...
import type { DocumentTypeDefinition, PrintingSettings } from "@carbon/printing";
import { labelSizes, type ProductLabelItem } from "@carbon/utils";
import { renderToStream } from "@react-pdf/renderer";
import type { SupabaseClient } from "@supabase/supabase-js";
import { logger, task } from "@trigger.dev/sdk";
import { printJobDeliver } from "./print-job-deliver";
...
export const printJob = task({
  ...
  run: async (payload: Payload) => {
    ...
    // Find all document types triggered by this source document
    ...
    for (const documentTypeId of documentTypeIds) {
      ...
      const printJobIds = await processDocumentType(client, {
        docType,
        sourceDocument,
        sourceDocumentId,
        companyId,
...
async function processDocumentType(
  ...
  if (printerRouteId) {
    const { data: route } = await getPrinterRoute(client, printerRouteId, companyId);
    if (route) {
  ...
  if (!resolver) {
    logger.warn(`No data resolver registered for document type: ${docType.id}. Add a resolver to the resolvers map in print-job.tsx.`);
    return [];
  }

  const resolved = await resolver(client, sourceDocument, sourceDocumentId, companyId);
  if (!resolved || resolved.items.length === 0) return [];

  // 3. Process each item: create job → render → update content → trigger delivery
  ...
  for (let i = 0; i < resolved.items.length; i++) {
    ...
    // Create job immediately so it appears in Print Manager
    ...
    if (job.error || !job.data) {
      logger.error(`Failed to create print job: ${job.error?.message}`);
      continue;
    }
    ...
    try {
      ...
      // Update job with rendered content (transitions to queued)
      ...
      logger.error(`Rendering failed for job ${jobId}: ${message}`);
      await updatePrintJobStatus(client, jobId, companyId, "failed", {
...
async function renderBuiltIn(
  ...
  switch (docType.id) {
    case "productLabel":
      return renderBuiltInProductLabel(
        resolved,
        format,
        mediaSizeId
      );
...
async function queryTrackedEntities(
  ...
  trackedEntities: Database["public"]["Tables"]["trackedEntity"]["Row"][] | null;
  readableId: string | null;
}> {
  switch (sourceDocument) {
    ...
    case "Operation": {
      const { data: jobOperation } = await client
        .from("jobOperation")
        .select("jobMakeMethodId, readableId")
        .eq("id", sourceDocumentId)
        .single();
      ...
      return {
        trackedEntities,
        readableId: jobOperation.readableId ?? null,
      };
    }
    case "Entity": {
  ...
  return trackedEntities
    .map((te) => {
      ...
      return {
        ...
        trackingType:
          (te.quantity ?? 1) > 1 ? "Batch" : ("Serial" as string),
      };
    })
