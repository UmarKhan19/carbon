    ...
    try {
      for (const [key, records] of Object.entries(byCompanyProvider)) {
        ...
        try {
          ...
          logger.error(`Failed to process sync for ${key}:`, { error: error instanceof Error ? error.message : String(error) });
          for (const r of records) {
            results.failed.push({
              recordId: r.event.recordId,
      ...
      logger.error("Sync task failed:", { error: error instanceof Error ? error.message : String(error) });
    } finally {
      await pool.end();
    }
