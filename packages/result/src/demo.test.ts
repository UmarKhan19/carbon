...
const SPANISH: Record<string, string> = {
  "error.notFound": "No se encontró {entity}",
  ...
  "error.database": "Algo salió mal al guardar sus cambios",
...
describe("@carbon/result — all errors and their translations", () => {
  ...
  it("NotFoundError interpolates the entity in every locale", () => {
    const notFound = rows.find((r) => r.tag === "NotFoundError");
    expect(notFound?.en).toBe("Approval request not found");
    expect(notFound?.es).toBe("No se encontró Approval request");
    expect(notFound?.de).toBe("Approval request wurde nicht gefunden");
  });
  ...
  it("DatabaseError shows a generic message and never leaks the raw error", () => {
    ...
    expect(db?.es).toBe("Algo salió mal al guardar sus cambios");
