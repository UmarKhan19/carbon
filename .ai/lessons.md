...
**Rule:** Don't manage TS2589 on Supabase joined-selects with `@ts-expect-error`. Break the deep inference at the source with a localized cast — `const result = await (client as any).from("supplierPart").select(...)` — when the result is only read for a few fields. It removes the file from the cumulative-depth pool entirely, so nothing flips. (Prefer a typed result annotation if the data is used broadly.)
