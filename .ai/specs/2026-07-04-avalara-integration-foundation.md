...
- [ ] **E-invoicing API credentials**: Avalara's e-invoicing (ELR) API appears to require Bearer tokens from Avalara Identity (OAuth2 client-credentials — i.e. separate `AVALARA_CLIENT_ID`/`AVALARA_CLIENT_SECRET`) rather than the AvaTax license key. Verify against current Avalara docs during Task 2; if confirmed, approve adding the two env vars. Blocks only the `einvoicing.ts` auth internals — AvaTax surface, config, hooks, and UI are unaffected.
